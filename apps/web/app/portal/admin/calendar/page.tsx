...
const SAFETY_CRITERIA: SafetyCriteria[] = [
  {
    id: 'wheelchair',
    name: 'Wheelchair Accessible',
    icon: 'User',
    description: 'Student uses wheelchair or mobility device',
  ...
  },
  {
    ...
    icon: 'Eye',
    description: 'Student has vision impairment',
  ...
  },
  {
    ...
    icon: 'Bell',
    description: 'Student has hearing impairment',
  ...
  },
  {
    ...
    icon: 'User',
    description: 'Student is on autism spectrum',
  ...
  },
  {
    ...
    name: 'Anxiety',
    icon: 'Heart',
    description: 'Student experiences anxiety',
  ...
  },
  {
    ...
    icon: 'AlertTriangle',
    description: 'Student has physical disability',
  ...
  },
  {
    ...
    icon: 'Shield',
    description: 'Student has medical condition requiring attention',
    color: 'bg-orange-100 text-orange-800'
  }
];

// Lesson type color coding
const LESSON_TYPE_COLORS: { [key: string]: string } = {
  'Beginner Riding': 'bg-blue-50 border-l-4 border-blue-500',
  'Advanced Riding': 'bg-purple-50 border-l-4 border-purple-500',
  'Intermediate Riding': 'bg-green-50 border-l-4 border-green-500',
  'Private 30': 'bg-yellow-50 border-l-4 border-yellow-500',
  'Private 60': 'bg-orange-50 border-l-4 border-orange-500',
  'Group 60': 'bg-pink-50 border-l-4 border-pink-500',
  'Ground Work': 'bg-indigo-50 border-l-4 border-indigo-500'
};
...
export default function AdvancedCalendar() {
  ...
  const editEvent = async (eventData: CalendarEvent) => {
    try {
      const response = await fetch(`/api/v1/admin/calendar-events/${eventData.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
      ...
      if (response.ok) {
        ...
        setEvents(events.map(event => 
          event.id === eventData.id ? updatedEvent.data : event
        ));
  ...
  const getSafetyIcon = (criteriaId: string) => {
    const criteria = SAFETY_CRITERIA.find(c => c.id === criteriaId);
    ...
    return iconMap[criteria.icon] || AlertTriangle;
  };

  const getEventDisplayText = (event: CalendarEvent) => {
    if (event.type === 'lesson' && event.student && event.lessonType) {
      return `${event.student.firstName} ${event.student.lastName} - ${event.lessonType.name}`;
    }
    return event.title;
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <h1 className="text-3xl font-bold text-gray-900 mt-2">Calendar & Scheduling</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="bg-white rounded-lg shadow p-4 mb-6">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <h3 className="text-sm font-medium text-gray-700 mb-2">Safety Considerations Legend:</h3>
            <div className="flex flex-wrap gap-3">
              {SAFETY_CRITERIA.map((criteria) => {
                const IconComponent = getSafetyIcon(criteria.id);
                return (
                  <div key={criteria.id} className="flex items-center space-x-2">
                    <div className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-white shadow-sm border">
                      {IconComponent && <IconComponent className="h-3 w-3 text-gray-600" />}
                    </div>
                    <span className="text-xs text-gray-600">{criteria.name}</span>
                  </div>
                );
              })}
            </div>
...
function DailyView({ 
  ...
  // Helper function for getting safety icons
  const getSafetyIcon = (criteriaId: string) => {
    const criteria = SAFETY_CRITERIA.find(c => c.id === criteriaId);
    if (!criteria) return null;
  ...
  // Create a timeline with events and available slots
  ...
  while (currentTime < timeSlots.length) {
    const currentSlotTime = timeSlots[currentTime];
    
    // Find events that start at this time
    const startingEvents = dayEvents.filter(event => event.startTime === currentSlotTime);
    ...
    if (startingEvents.length > 0) {
      ...
      startingEvents.forEach(event => {
        ...
        // Skip the time slots covered by this event
      ...
      <div className="divide-y divide-gray-200">
        {timeline.map((item, index) => {
          if (item.type === 'event' && item.event) {
            ...
            const safetyIcons = event.safetyCriteria?.map(criteriaId => {
              const criteria = SAFETY_CRITERIA.find(c => c.id === criteriaId);
              const IconComponent = getSafetyIcon(criteriaId);
              return { criteria, IconComponent };
            ...
            return (
              <div 
                ...
                className={`px-6 py-3 cursor-pointer ${
                  event.type === 'lesson' && event.lessonType?.name
                    ? LESSON_TYPE_COLORS[event.lessonType.name] || 'bg-gray-50'
                    : event.type === 'event'
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        {/* Safety Icons - Only icons, no text */}
                        {safetyIcons.map(({ criteria, IconComponent }) => (
                          criteria && IconComponent && (
                            <div
                              key={criteria.id}
                              className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-white shadow-sm"
            ...
            return (
              <div 
                key={`available-${item.time || 'unknown'}`}
          ...
          <div key={dateString} className="border-r border-gray-200 last:border-r-0">
            ...
            <div className="relative" style={{ height: `${timeSlots.length * 48}px` }}>
              ...
              {timeSlots.map((time, timeIndex) => (
                <div 
                  key={time}
                  className={`absolute left-0 right-0 border-b border-gray-200 px-2 py-1 cursor-pointer ${
                ...
                >
                  <div className="text-xs text-gray-400 opacity-0">Available</div>
                </div>
              ))}
              
              {/* Render events */}
              {dayEvents.map((event) => {
                // Find the start and end time indices
                const startTimeIndex = timeSlots.findIndex(slot => slot >= event.startTime);
                const endTimeIndex = timeSlots.findIndex(slot => slot >= event.endTime);
                
                // Calculate position and height
                const top = startTimeIndex >= 0 ? startTimeIndex * 48 : 0;
                const duration = endTimeIndex > startTimeIndex ? endTimeIndex - startTimeIndex : 1;
                const height = Math.max(duration * 48, 48); // Minimum height of 48px
                ...
                return (
                  <div 
                    ...
                    className={`absolute left-0 right-0 px-2 py-1 rounded text-xs font-medium truncate cursor-pointer z-10 ${
                      event.type === 'lesson' && event.lessonType?.name
                        ? LESSON_TYPE_COLORS[event.lessonType.name] || 'bg-gray-100 text-gray-800'
                        : event.type === 'event'
      ...
      <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div className="mt-3">
          ...
          <div className="space-y-4">
            ...
            {event.type === 'lesson' && event.student && (
              <div>
                <span className="font-medium text-gray-700">Student:</span>
                <p className="text-gray-900">{event.student.firstName} {event.student.lastName}</p>
              </div>
            )}
            
            {event.type === 'lesson' && event.lessonType && (
              <div>
                <span className="font-medium text-gray-700">Lesson Type:</span>
                <p className="text-gray-900">{event.lessonType.name} ({event.lessonType.durationMinutes} min)</p>
              </div>
            )}
            ...
            {event.horseName && (
              <div>
                ...
                <p className="text-gray-900">{event.horseName}</p>
              </div>
            )}
            ...
            {event.instructorName && (
              <div>
                ...
                <p className="text-gray-900">{event.instructorName}</p>
              </div>
            )}
            
            {event.safetyCriteria && event.safetyCriteria.length > 0 && (
              <div>
                <span className="font-medium text-gray-700">Safety Considerations:</span>
                <div className="flex flex-wrap gap-2 mt-2">
                  {event.safetyCriteria.map((criteriaId) => {
                    const criteria = SAFETY_CRITERIA.find(c => c.id === criteriaId);
                    const IconComponent = getSafetyIcon(criteriaId);
                    ...
                    return (
                      <div
                        key={criteriaId}
                        className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${criteria?.color || 'bg-gray-100 text-gray-800'}`}
                        title={criteria?.description}
                      >
            ...
            <div className="flex items-center">
              <input
                type="checkbox"
                checked={event.blocksTime}
                disabled
                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
              />
              <label className="ml-2 block text-sm text-gray-900">
                Blocks time for other lessons
              </label>
            </div>
          </div>
...
// Helper function for getting event display text
function getEventDisplayText(event: CalendarEvent) {
  if (event.type === 'lesson' && event.student && event.lessonType) {
    return `${event.student.firstName} ${event.student.lastName} - ${event.lessonType.name}`;
  }
  return event.title;
